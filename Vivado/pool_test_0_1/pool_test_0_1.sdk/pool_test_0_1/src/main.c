...
#include "xcnn_pool_d16x16_p2x2.h"
#include "xaxidma.h"


struct cnn_hw {
	XCnn_pool_d16x16_p2x2 cnn_0;
	XCnn_pool_d16x16_p2x2_Config *cnn_0_cfg;
	XAxiDma axiDMA;
...
int cnn_hw_init(struct cnn_hw *p_cnn_hw)
{
	...
	p_cnn_hw->cnn_0_cfg = XCnn_pool_d16x16_p2x2_LookupConfig(XPAR_CNN_POOL_D16X16_P2X2_0_DEVICE_ID);
	if (!p_cnn_hw->cnn_0_cfg) {
		printf("XCnn_LookupConfig failed\n");
		return XST_FAILURE;
	}

	status = XCnn_pool_d16x16_p2x2_CfgInitialize(&p_cnn_hw->cnn_0, p_cnn_hw->cnn_0_cfg);
...
void XCnn_Set_HW(XCnn_pool_d16x16_p2x2 *InstancePtr, u32 ctrl)
{
	XCnn_pool_d16x16_p2x2_Set_ctrl(InstancePtr, ctrl);
}
...
int main()
{
	...
	do {
		...
		XCnn_pool_d16x16_p2x2_Start(&cnn_hw.cnn_0);
		...
		while (!XCnn_pool_d16x16_p2x2_IsDone(&cnn_hw.cnn_0))
			/* wait */ ;

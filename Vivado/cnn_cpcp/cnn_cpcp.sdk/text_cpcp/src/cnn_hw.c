...
void cnn_hw_eval(struct cnn_hw *cnn_hw, struct cnn_run *cnn_run)
{
	...
	Xil_DCacheFlushRange((uint32_t) cnn_hw->p_dma_buffer_RX, POOL_1_OUTPUT_LEN * sizeof(uint32_t));
	...
	XAxiDma_SimpleTransfer(&cnn_hw->axiDMA, (uint32_t)cnn_hw->p_dma_buffer_RX, POOL_1_OUTPUT_LEN * sizeof(uint32_t), XAXIDMA_DEVICE_TO_DMA);
	while (XAxiDma_Busy(&cnn_hw->axiDMA, XAXIDMA_DEVICE_TO_DMA));
	Xil_DCacheInvalidateRange((uint32_t) cnn_hw->p_dma_buffer_RX, POOL_1_OUTPUT_LEN * sizeof(uint32_t));

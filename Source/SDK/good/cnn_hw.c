...
#if (PLATFORM == FPGA)
void (*XCnn_Set_conv_0_kernel[CONV_0_KERNEL_LEN])(XCnn_conv_A *InstancePtr, uint32_t Data) = {
		XCnn_conv_A_Set_kernel_0,
		XCnn_conv_A_Set_kernel_1,
		XCnn_conv_A_Set_kernel_2,
		XCnn_conv_A_Set_kernel_3,
		XCnn_conv_A_Set_kernel_4,
		XCnn_conv_A_Set_kernel_5,
		XCnn_conv_A_Set_kernel_6,
		XCnn_conv_A_Set_kernel_7,
		XCnn_conv_A_Set_kernel_8,
};

void (*XCnn_Set_conv_1_kernel[CONV_1_KERNEL_LEN])(XCnn_conv_B *InstancePtr, uint32_t Data) = {
		XCnn_conv_B_Set_kernel_0,
		XCnn_conv_B_Set_kernel_1,
		XCnn_conv_B_Set_kernel_2,
		XCnn_conv_B_Set_kernel_3,
		XCnn_conv_B_Set_kernel_4,
		XCnn_conv_B_Set_kernel_5,
		XCnn_conv_B_Set_kernel_6,
		XCnn_conv_B_Set_kernel_7,
		XCnn_conv_B_Set_kernel_8,
};

void (*XCnn_Set_conv_2_kernel[CONV_2_KERNEL_LEN])(XCnn_conv_C *InstancePtr, uint32_t Data) = {
		XCnn_conv_C_Set_kernel_0,
		XCnn_conv_C_Set_kernel_1,
		XCnn_conv_C_Set_kernel_2,
		XCnn_conv_C_Set_kernel_3,
		XCnn_conv_C_Set_kernel_4,
		XCnn_conv_C_Set_kernel_5,
		XCnn_conv_C_Set_kernel_6,
		XCnn_conv_C_Set_kernel_7,
		XCnn_conv_C_Set_kernel_8,
};
#endif
...
int cnn_hw_init(struct cnn_hw *p_cnn_hw)
{
	...
	status = XCnn_conv_A_Initialize(&p_cnn_hw->conv_0, XPAR_CNN_CONV_A_0_DEVICE_ID);
	if (status != XST_SUCCESS)
		return status;

	status = XCnn_pool_a_Initialize(&p_cnn_hw->pool_0, XPAR_CNN_POOL_a_0_DEVICE_ID);
	if (status != XST_SUCCESS)
		return status;

	status = XCnn_conv_B_Initialize(&p_cnn_hw->conv_1, XPAR_CNN_CONV_B_0_DEVICE_ID);
	if (status != XST_SUCCESS)
		return status;

	status = XCnn_pool_b_Initialize(&p_cnn_hw->pool_1, XPAR_CNN_POOL_b_0_DEVICE_ID);
	if (status != XST_SUCCESS)
		return status;

	status = XCnn_conv_C_Initialize(&p_cnn_hw->conv_2, XPAR_CNN_CONV_C_0_DEVICE_ID);
	if (status != XST_SUCCESS)
		return status;

	status = XCnn_pool_c_Initialize(&p_cnn_hw->pool_2, XPAR_CNN_POOL_c_0_DEVICE_ID);
	if (status != XST_SUCCESS)
		return status;
...
void XCnn_Set_Conv_0(XCnn_conv_A *InstancePtr, uint32_t ctrl, uint32_t *kernel)
{
	XCnn_conv_A_Set_ctrl(InstancePtr, ctrl);
	for (int i = 0; i < CONV_0_KERNEL_LEN; i++)
		XCnn_Set_conv_0_kernel[i](InstancePtr, kernel[i]);
}

void XCnn_Set_Conv_1(XCnn_conv_B *InstancePtr, uint32_t ctrl, uint32_t *kernel)
{
	XCnn_conv_B_Set_ctrl(InstancePtr, ctrl);
	for (int i = 0; i < CONV_1_KERNEL_LEN; i++)
		XCnn_Set_conv_1_kernel[i](InstancePtr, kernel[i]);
}

void XCnn_Set_Conv_2(XCnn_conv_C *InstancePtr, uint32_t ctrl, uint32_t *kernel)
{
	XCnn_conv_C_Set_ctrl(InstancePtr, ctrl);
	for (int i = 0; i < CONV_2_KERNEL_LEN; i++)
		XCnn_Set_conv_2_kernel[i](InstancePtr, kernel[i]);
}

void XCnn_Set_Pool_0(XCnn_pool_a *InstancePtr, uint32_t ctrl)
{
	XCnn_pool_a_Set_ctrl(InstancePtr, ctrl);
}

void XCnn_Set_Pool_1(XCnn_pool_b *InstancePtr, uint32_t ctrl)
{
	XCnn_pool_b_Set_ctrl(InstancePtr, ctrl);
}

void XCnn_Set_Pool_2(XCnn_pool_c *InstancePtr, uint32_t ctrl)
{
	XCnn_pool_c_Set_ctrl(InstancePtr, ctrl);
}
...
{
	XCnn_pool_c_Start(&cnn_hw->pool_2);
	XCnn_conv_C_Start(&cnn_hw->conv_2);
	XCnn_pool_b_Start(&cnn_hw->pool_1);
	XCnn_conv_B_Start(&cnn_hw->conv_1);
	XCnn_pool_a_Start(&cnn_hw->pool_0);
	XCnn_conv_A_Start(&cnn_hw->conv_0);
...
{
	while (!XCnn_pool_c_IsDone(&cnn_hw->pool_2));

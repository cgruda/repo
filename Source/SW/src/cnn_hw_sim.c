			...
			for (int n = 0; n < pool_dim; n++) {
				for (int m = 0; m < pool_dim; m++) {
					if (input[((pool_dim * i) + n) * input_cols + ((j * pool_dim) + m)] > max_val) {
						max_val = input[((pool_dim * i) + n) * input_cols + ((j * pool_dim) + m)]; // FIXME: this might be the bug in hw
					}
...
void fp_fully_connected(uint32_t *input, uint32_t *weight, uint32_t *bias, uint32_t *output, uint32_t ctrl)
{
	...
	for (int i = 0; i < input_len; i++) {
		for (int j = 0; j < output_len; j++) {
			// if (j == 0) {
			// 	fixed_point_print(weight[i * weight_cols + j]);
			// 	PRINT_UI(" * ");
			// 	fixed_point_print(input[i]);
			// 	PRINT_UI(") + ");
			// }
...
void cnn_hw_sim_conv_0_set(struct cnn_hw_sim *cnn_hw_sim, uint32_t ctrl, float *kernel)
{
	...
	// fp_cnn_config_trace_vals("hw_conv_0_kernel:", cnn_hw_sim->conv_0_kernel, CONV_0_KERNEL_DIM, CONV_0_KERNEL_DIM);
}
...
void cnn_hw_sim_conv_1_set(struct cnn_hw_sim *cnn_hw_sim, uint32_t ctrl, float *kernel)
{
	...
	// fp_cnn_config_trace_vals("hw_conv_1_kernel:", cnn_hw_sim->conv_1_kernel, CONV_1_KERNEL_DIM, CONV_1_KERNEL_DIM);
}
...
void cnn_hw_sim_fc_0_set(struct cnn_hw_sim *cnn_hw_sim, uint32_t ctrl, float *weight, float *bias)
{
	...
	// fp_cnn_config_trace_vals("hw_fc_0_bias:", cnn_hw_sim->fc_0_bias, 1, FC_0_BIAS_LEN);
	...
	// fp_cnn_config_trace_vals("hw_fc_0_weight:", cnn_hw_sim->fc_0_weight, FC_0_WEIGHT_ROWS, FC_0_WEIGHT_COLS);
}
...
void cnn_hw_sim_fc_1_set(struct cnn_hw_sim *cnn_hw_sim, uint32_t ctrl, float *weight, float *bias)
{
	...
	// fp_cnn_config_trace_vals("hw_fc_1_bias:", cnn_hw_sim->fc_1_bias, 1, FC_1_BIAS_LEN);
	...
	// fp_cnn_config_trace_vals("hw_fc_1_weight:", cnn_hw_sim->fc_1_weight, FC_1_WEIGHT_ROWS, FC_1_WEIGHT_COLS);
}
...
void cnn_hw_sim_eval(struct cnn_hw_sim *cnn_hw_sim, struct cnn_run *cnn_run)
{
	...
	// 	if (cnn_hw_sim->fc_0_output[i]) {
	// 		PRINT_UI("[%d]  ", i);
	// 		fixed_point_print(cnn_hw_sim->fc_0_output[i]);
	// 		PRINT_UI("\r\n");
	// 	}
	// }
	...
	//print_fixed_arr("fc_1_out:", cnn_hw_sim->fc_1_output);
	...
	//print_float_arr("softmax:", cnn_hw_sim->output_data);
